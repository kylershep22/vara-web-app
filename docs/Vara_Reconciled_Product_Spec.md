...
11. **`weekStart` is a provisional open-date derivation — open, from the weekly-open slice (f080afa).** `weekStart` is written as the ISO date at the moment of weekly open, and "already open this week" is any cycle whose `weekStart` is strictly < 7 days old. Deliberate for launch scope (avoids pulling the week-start-day picker in from Section 18 onboarding), but it means `weekStart` is **not** a stable anchor day — it drifts to whatever day the user opens on. A later slice reading `weekStart` as a fixed weekly boundary must not assume stability until Section 18 onboarding captures `weekStartDay` and the derivation switches to it. Recorded in `utils/weekStart.ts` header.
