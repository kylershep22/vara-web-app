...
11. **`weekStart` is a provisional open-date derivation — RESOLVED (Step 3a-i, weekly-boundary parameterization).** Was: `weekStart` written as the ISO date at the moment of weekly open, with "already open this week" being any cycle whose `weekStart` is strictly < 7 days old — so `weekStart` drifted to whatever day the user opened on and a chosen start day could never take effect. Resolved by parameterizing the boundary on `userPrivate.weekStartDay` instead of on a fixed length. Three parts: (a) `weeklyCycles.weekEnd` is now **stored** on the document (ISO, inclusive), so a live week's boundary is a fact a later `weekStartDay` change cannot retroactively move; (b) the entry guard resolves on `todayIso <= weekEnd` rather than `age < 7`, which is what lets the first cycle be a partial **stub** (setup day → day before the next start day) without keeping it current into the following week; (c) the write path anchors recurring weeks to `weekStartDay` via `planWeek`, so a late open belongs to the week that already began rather than starting a new one. Cycles written before the field have no `weekEnd` and fall back to `weekStart + 6` — identical to the old behavior, so **no backfill migration**. With no `weekStartDay` set (the pre-picker state), `planWeek` anchors on the open date and behavior is unchanged. The Section 18 picker that captures `weekStartDay` is Step 3a-ii. Recorded in `utils/weekStart.ts` header.
12. **`weeklyCycles` composite index absent — open, from the weekly-open slice (f080afa).** `getRecentWeeklyCycles` (`userId ==` + `orderBy weekStart desc`) requires a composite index that does not exist in `firestore.indexes.json` and fails against production. The function is unused as of the weekly-open and re-set slices and carries an in-code warning. The weekly-close / continuity slice is its first real caller and **must** add the index to `firestore.indexes.json` and deploy it before using `getRecentWeeklyCycles` in prod.
13. **Weekly loop reachable only via the `__DEV__` Settings entry — open, from the weekly-open slice (f080afa).** The floor → open → Today flow is mounted behind the "Developer (dev builds only)" Settings row and is absent from preview/production builds. Deliberate reachable-not-default staging; the real entry arrives when Section 18 progressive onboarding replaces `ONBOARDING_V2` and routes users into the weekly loop. Until then the loop has no production entry point by design.
