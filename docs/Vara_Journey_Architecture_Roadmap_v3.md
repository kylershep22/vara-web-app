...
### 2026-09-14 - R2 built and walked: the bar floats, and sixteen routes discover they were never clearing it (branch `design/slice-r2-floating-nav`, ELEVEN commits, **UNMERGED**, **SECTION A WALKED IN FULL on an iPhone 14 Plus, fifteen steps passed; Sections B and C NOT RUN**, plan and results at `docs/walks/r2/WALK.md`)
...
| walk | this commit | Section A results, the `#6F7F77` closure, the R2 row |

**THREE OF THE ELEVEN ARE WALK TUNES, WHICH IS THE GATE DOING ITS JOB RATHER THAN THE
...
**(3) A5b, one look at two controls.** The `Conversations` FAB was the single hard
collision Step 0 identified, and `ReportDetail`'s sticky block hardcoded `34`. A5 covers
scroll content and neither of these is a "last item". Both changed in `6e9a590`; both
plausibly fine; plausibly fine is what a walk replaces.
