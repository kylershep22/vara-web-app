...
### 2026-09-16 - NEW-MESSAGE-SHEET built: the sheet could not shrink, so it left the screen instead (branch `fix/new-message-sheet`, `7f415f7` the fix, `6deb614` the tests, this entry the docs; **BEFORE-STATE CAPTURE RUN AND PASSED on `main` — steps 0, 0b and 0c, iPhone 14 Plus, and the mechanism is CONFIRMED ON HARDWARE; the branch half of the walk is OUTSTANDING; UNMERGED**, script and results at `docs/walks/new-message-sheet/WALK.md`)
...
**THE SURFACE HAD NO WORKING DISMISSAL AT ALL, AND THE BACKDROP FAILED FOR A REASON NOBODY WOULD GUESS.** Close control off-screen; `PanResponder` spread on the handle strip, also off-screen; `onRequestClose` is Android-only for a `transparent` modal with no `allowSwipeDismissal`. And **tap-outside**: the overlay was `absoluteFillObject` INSIDE the KAV, and Yoga resolves absolute insets against the parent's **padding** box, so the keyboard's padding shrank the backdrop to exactly the region the sheet already covered. **Not one exposed pixel.** Kyle's "cannot be dismissed without backgrounding the app" is confirmed affordance by affordance rather than taken on report. **One undesigned escape exists**: the shared `TextInput` sets no `blurOnSubmit`, so RN's single-line default of `true` means the keyboard's Search key blurs the field and drops the sheet back into view. An unlabelled keypress is not a dismiss control and it changed nothing about the fix - **but it is the falsifier for the entire diagnosis**, so it is walk step 0c and it runs before anything else.
...
**BEFORE-STATE CAPTURE RUN AND PASSED, STEPS 0 / 0b / 0c, iPhone 14 Plus, on `main` at `1cc0746` (Kyle).** *(Attested 2026-09-14 per the report. The walk script these steps are numbered against was committed 2026-09-16, so if the sitting was that day the date moves; **flagged rather than silently corrected, because an attestation is the walker's words** - the same treatment R2's row gave the addendum it could not find in the repository.)*
