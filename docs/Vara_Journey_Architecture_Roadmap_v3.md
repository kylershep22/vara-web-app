...
**NOT WALKED YET.** The walk is a seeded malformed row: set `phaseKey` to
`"remove "` with a trailing space in the console, cold open, expect Today on the
weekly landing with no crash and ONE warn line carrying an 8-character digest
and not the uid; then restore, cold open, and expect the journey surfaces back.
**A trap for whoever walks it:** if you broke `destination` rather than
