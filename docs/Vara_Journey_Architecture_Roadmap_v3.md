...
| 8 | **[DONE `8621ec9`, 2026-09-18; suites attested 2026-09-18; walked 19 of 23, 14 Plus, 3 FAILs (swipe-to-dismiss, see `PANRESPONDER-VERIFICATION`), 1 NOT RUN (offline failure, not reachable on a dev client)]** *(branch `journey/slice-8-good-moments`, merged `--no-ff` 2026-09-18, six commits from `2185d83` to `b78a97d`; branch not deleted. **THE FAILURES ARE ON THE MARKER RATHER THAN BURIED IN §13, DELIBERATELY: anyone reading this board should see that this row ships with a known broken gesture without having to open the build log.** **THE MARKER READ `BUILT` AND `UNMERGED` UNTIL THIS COMMIT, AND THAT WAS NOT AN OVERSIGHT:** a marker cannot carry a hash for a commit that does not exist yet, so `DONE` plus the merge hash lands in the docs commit that FOLLOWS the merge - the same order slice 7c used (`247cdc4` on the branch, then the merge, then `b240133` on `main`). **THE `[Next]` MARKER BELOW IS NOW UNRESOLVED AND IS DELIBERATELY LEFT WHERE IT IS.** Its own condition - *it stays here until this row merges* - has been met, so it is owed a move. **It has not been moved, because where it goes is a board decision rather than a bookkeeping one:** table order puts row 9 next, and row 9 carries NO marker at all and is under a standing instruction not to be given an invented one. Promoting it is Kyle's call. Recorded here rather than left silent, so the board does not quietly acquire two live markers or none. **"SUITES ATTESTED" WAS REMOVED FROM THIS MARKER ON 2026-09-18 AND THE REMOVAL IS A RULE, NOT A CORRECTION TO THIS ROW: CC running the suites and reporting the numbers is EVIDENCE; ATTESTATION IS A GATE ACTION KYLE PERFORMS EXPLICITLY, and a marker cannot claim it on his behalf. The phrase goes back at the merge docs commit, dated the day he says it. Markers on earlier rows that carry the phrase are left alone - they are records of what was written at the time, and rewriting them would be inventing an attestation history rather than correcting one.**)* *(promoted at 7c's merge, 2026-09-17; the ONE live marker on this board, confirmed by reading every row rather than by recalling which was last moved. The row carried NO status at all before this - the same drift rows 1 and 3 were corrected for on 2026-09-10 - so this is a marker arriving rather than moving. Row 9 still carries none and is left alone rather than given an invented one.)* **RESOLVED 2026-09-19. The marker is moved to `R1A-SECTION-B-WALK`, a new row placed after R2. It did not go to row 9: table order is not a reason, row 9 still carries no marker, and the standing instruction against inventing one for it is unchanged. This row keeps everything else it says, including its three FAILs.** **Good moments** | `moments/{uid}_{ts}` collection (rules, deleteAccount), one-tap entry sheet from D1 below-fold row, single-line input, no list surface on Today; feeds nothing until Insights ships. **IN SCOPE, ADDED 2026-09-12 (Kyle's correction to the Step-0 toast finding): `showNotificationToast` RETURNS SILENTLY when an unlock toast is visible** (`ToastContext.tsx:130`), so a successful save would confirm nothing. **That is not a caveat, it is the case Jen's copy exists to prevent** - her whole reason for wanting "Saved." is removing uncertainty about whether the save landed, and a success that shows nothing is precisely the uncertainty she was designing against. A silent success is also WORSE than no toast at all, because the user has been told elsewhere to expect one. Slice 8 owns the fix: a fallback path, a queue for this toast class, or a different confirmation surface. **Also settle:** the API takes a title AND a body, and "Saved." is title-only, so decide what a title-only notification toast renders as rather than passing an empty string and finding out on device. **The toast CANNOT stack and CANNOT count** - verified at Step 0, `showNotificationToast` holds one object rather than a queue, and the queue that does stack is the separate feature-unlock path - so that half needs nothing. | rules; ~~**[Content-gated]** copy~~ **GATE CLEARED 2026-09-12: Jen delivered the prompt, the "Saved." line and the failure line (see the 2026-09-12 Jen-feedback entry, item 3). Marker struck rather than deleted because contradiction (E) in that entry predicted it would be found stale in three places; this is one of them, and the rename of this row from "Moments of joy" to "Good moments" plus the other two citations are still row 8's own to do.** | Yes |
| 9 | **[SPLIT 2026-09-19 into 9.1a, 9.1b, 9.2 and 9.3; see the AMENDED block below. This row carries no marker and is not given one - it no longer names work, only the group.]** Behavioral protocol screen + remind-later | The Daily Action Launcher behavioral screen (protocol, why, mark done, remind me later) for `remove` protocols; one-off later-today notification (`scheduleLocalNotification` DATE trigger), `scheduledAt` on `DailyLog`, third card state, cancellation bookkeeping; OS-settings redirect after denial. | **None outstanding. The completion-semantics decision is §9 item 7, RESOLVED 2026-09-10 as user-declared or naturally completed (§9 R7).** *(Citation corrected 2026-09-21: this cell read "Completion semantics decision (mockup v1 E1 open item)" and no such item exists anywhere in the repo - see the AMENDED block below.)* | Yes |
| 9.1a | **[DONE `cb2eb0f`, 2026-09-21; suites attested 2026-09-21; walked 11 of 17 on a 14 Plus, ZERO FAILs, 6 NOT RUN (4 need a fresh uncompleted day, 1 needs five consistent days, 1 depends on it). Section C ran: all four provenance fields verified in the console against the protocol that was on screen, and the historical row proven untouched.]** Completion carries provenance: four additive fields, the write path, and the historical-read contract *(row added 2026-09-21)* | Four additive optional fields on `DailyLog` - `completedAt`, `completionSource`, `protocolCellId`, `protocolFamily` - written by the EXISTING `markDone` from the EXISTING Today card. **No new screen, no new route, no new navigation.** `protocolCompleted` becomes optional in TypeScript to match what is persisted. **Provenance is gated as a unit by `stampProvenance`, not field by field.** A shared test fixture, the four historical scenarios, and three documentation corrections. | None. The completion-semantics decision was discharged at §9 R7 on 2026-09-10. | **Yes, and Section C in the Firestore console is acceptance, not a nicety** - the payload is invisible on device. |
| ROLLOVER-SAFETY | **[DONE `d22d913`, 2026-09-21; suites attested 2026-09-21; walked 9 of 9 on a 14 Plus, ZERO FAILs. The red test reproduced the defect on unmodified code before any fix existed.]** A completion can carry yesterday's protocol identity into today's row *(row added 2026-09-21)* | One date stamp committed in the same React batch as `setProtocol`, derived during render as `staleDate`, consumed by a render-time check on the CTA and a completion-boundary guard in `markDone`. **Date only, never identity.** Fifteen additive edits across three files. **Does NOT touch `activeRef`, `stampProvenance`, the completion patch shape, or `useTodayCard`'s load effect dependencies.** | None. | **Yes, and it is regression plus one observation.** The defect is not reproducible by hand and the walk cannot pass or fail the fix; the automated coverage is the correctness evidence. |
| ASYNC-LOAD-OWNERSHIP | **[READY. BEFORE 9.1b. NOT AN R-SERIES ROW. OWN SLICE. Row added 2026-09-21 from ROLLOVER-SAFETY's Step 0.]** A superseded `useTodayCard` load can commit its stale state over the authoritative one *(row added 2026-09-21)* | `activeRef` is a single shared boolean. React runs the previous cleanup before the next effect body, so a superseded load's cleanup sets it false and its successor sets it true again, after which the superseded load's awaits resolve, find the flag true, and commit stale `protocol`, `picked` and `completed`. **A boolean cannot distinguish unmounted from superseded, and there is NO CLOSING EDGE:** the card can display a superseded protocol indefinitely with a live CTA. Replace the shared boolean with a per-run mechanism that lets each load determine whether it is still authoritative. **`markDone` (`:571`, `:575`) and `confirmPick` (`:618`, `:622`) read the same ref, so the cancellation contract is load-bearing in three places and must be preserved.** A superseded run's error or finally path must not overwrite a newer run's loading state either. **ROLLOVER-SAFETY mitigates this path's WRITE consequence only; the stale-state commit is unrepaired.** | None. | **Yes.** Requires out-of-order resolution coverage: load A starts, load B supersedes it, B resolves first and commits, A resolves after and must not overwrite. Both same-day and cross-date. |
...
| R1A-SECTION-B-WALK | **[WALKED PARTIAL 2026-09-19, 14 Plus, default and 1.3x; attested 2026-09-19. Steps 1, 3 and 13 NOT RUN. Three subjects moot: habits are out of the app. SE half NOT WALKABLE IN THIS SETUP. Step 14 clause 3 resolved by desk check. R3's gate (b) cleared by Kyle's ruling of 2026-09-19 on the partial, NOT by a full walk; see the dated block below.]** *(was **[Next]**, promoted from row 8 at `ee63eab`, 2026-09-19; the marker moves to no row at this commit and the board carries NO live [Next] until Kyle promotes one.)* **[NOT AN R-SERIES ROW. OWN SLICE. Row added 2026-09-19.]** Section B, R1a's fourteen steps, has never been run and has slipped four times *(row added 2026-09-19)* | **THIS ROW EXISTS BECAUSE THE WORK WAS INVISIBLE.** Section B has lived inside R2's marker and R3's gate (b) since 2026-09-14, owned by no row, and it has been deferred four times. Fourteen unrun steps gating R3 belong on the board where a reader deciding what to start next will see them. **WHAT IT IS.** The fourteen steps at `docs/walks/r1a/WALK.md`, which is the **CANONICAL** copy. A second verbatim copy of the same fourteen exists at `docs/walks/r2/WALK.md:512` and an amendment to one does not reach the other; `r1a/WALK.md` wins on any difference. **SCOPED LARGE END ONLY, KYLE'S RULING 2026-09-19.** The iPhone 14 Plus stands in for the iPhone 16 Pro Max per standards §18(d), which records it as within 2pt and covering the large end for every width-sensitive assertion. The SE half of all fourteen is a standing gap, recorded as **NOT WALKABLE IN THIS SETUP** and never as **NOT RUN**, consistent with §18(d) and with R3's existing carve-out. **WHY THE RULING WAS NEEDED AT ALL:** `WALK.md`'s header bound all fourteen steps to both matrix devices at both type sizes, unqualified, which made the gate unsatisfiable on any hardware present. That is a better account of four slips than the screenshot chore is. **ONE SUBSTITUTION IS NOT WIDTH-SENSITIVE AND IS RECORDED AS SAFE RATHER THAN EQUIVALENT.** Step 10 asks whether a card grew enough under Inter to push another below the fold, which is viewport HEIGHT: the 14 Plus is 926pt against the 16 Pro Max's 932pt. A 14 Plus shows the same or fewer cards above the fold, so a pass there implies a pass on the taller device. The direction is conservative and that is why the substitution holds here; it is not an equivalence and should not be quoted as one. **STEP 14 CLAUSE 3 IS OFF THIS WALK**, resolved by desk check 2026-09-19 and recorded in `WALK.md` at that step. Clauses 1 and 2 remain in scope on the 14 Plus. **COMPLETABILITY UNDER THE RULING:** twelve steps complete on a 14 Plus (1-9, 11-13); step 10 completes large-end only and only once its three captures exist; step 14 completes on clauses 1 and 2. The small end of every step is the standing gap; no step is SE-exclusive. | The three before-state captures at `0091ce5`, listed in `docs/walks/r1a/README.md`. Step 10 cannot run without them and they are Kyle's to take. Nothing else gates this row. | Yes, and the row is the walk. Fourteen steps, 14 Plus, default and 1.3x Dynamic Type, per `docs/walks/r1a/WALK.md`. |
...
> **AMENDED 2026-09-19 (the walk). THIS ROW CLOSES PARTIAL AND THE MARKER SAYS SO RATHER THAN
> BURYING IT.** Ten of the fourteen steps were walked or exercised on a 14 Plus. Steps 1, 3 and
> 13 were not run and remain runnable on the device present. Three subjects went moot mid-walk:
> habits are no longer accessible and will not ship, which retires `HabitDetailScreen` from
> steps 4 and 8 and `WeeklyHabitGrid` from step 9. **THE TWO HIGHEST-RISK STEPS BOTH PASSED, AND
> THAT IS THE RESULT THIS ROW EXISTED TO PRODUCE:** `LoginScreen`'s Paper text and the paywall's
> weight pairing had never been seen rendered under the text primitive, and both hold at 1.3x.
> **THE THREE UNRUN STEPS ARE NOT THE RISK THE GATE WAS PROTECTING AGAINST.** A diagnostic
> screen, a splash-timing check and a motion-preference check do not test whether Inter's
> metrics broke a shipped layout, which is what R1a merging unwalked under R2 put at stake. They
> stay owed and are cheap; they do not gate R3.

> **AMENDED 2026-09-19. GATE (b) IS CLEARED BY RULING, NOT BY A FULL WALK, AND THE DISTINCTION
> IS THE POINT.** Gate (b) required that Section B had been walked. Section B was walked
> **PARTIAL** on 2026-09-19: ten of fourteen steps, 14 Plus, default and 1.3x, with steps 1, 3
> and 13 not run and three subjects moot. Kyle ruled the same day that the partial is sufficient
> to start R3. **THE REASONING, RECORDED SO THE RULING CAN BE EVALUATED RATHER THAN JUST
> FOUND.** The gate existed because R1a applied Inter and the 1.3x cap to every `Text` in the
> app and merged unwalked, R2 restyled navigation on top of it, and R3 would be a third layer on
> two unverified ones. The risk was a metrics change having broken a shipped layout invisibly.
> The walk addressed exactly that: the two highest-risk surfaces were seen rendered for the
> first time and both passed, and the 1.3x clipping and container checks passed on every surface
> that still exists. The three unrun steps test other things. **GATE (a), THE REGENERATED
> BACKGROUND ASSET, IS UNAFFECTED AND STILL OPEN.** `mobile/assets/images/todayBackground.webp`
...
> **AMENDED 2026-09-21. THE SEQUENCE BEFORE R3 IS NOW FOUR ITEMS, NOT ONE, AND THE REASON IS
> RECORDED SO NOBODY LATER REMOVES A DEPENDENCY THAT LOOKS UNRELATED.** Order:
> **ROLLOVER-SAFETY (done, `d22d913`) → ASYNC-LOAD-OWNERSHIP → 9.1b → R3.** Both fixes are
> justified by **confirmed correctness defects in `useTodayCard`**, not by preference.
> ROLLOVER-SAFETY's defect **PREDATES 9.1a**; what 9.1a changed is the **CONSEQUENCE**, because
> a stale completion now corrupts persisted protocol identity rather than only completion state.
> **9.1a is NOT a technical prerequisite for either fix.** ASYNC-LOAD-OWNERSHIP blocks 9.1b
> because a persistent detail sheet must not inherit a hook that can display a superseded
> recommendation indefinitely.
>
> **THE SCOPE RULE, VERBATIM, SO THIS DOES NOT BECOME A GENERAL TODAY REFACTOR:**
>
> > "Only defects that threaten the correctness of the daily protocol interaction or persisted
> > completion data should create additional blockers before R3."
>
> Anything else found during implementation is recorded and prioritised separately.
>
> **A SEPARATE FOLLOW-UP, NOT A BLOCKER:** whether rows corrupted by ROLLOVER-SAFETY's defect
> already exist in production. The signature is `completed: true` with no `dailyTimeBudget`.
> **DO NOT REPAIR ON THAT SIGNATURE ALONE** - the 9.1a read found four distinct historical
> `DailyLog` shapes, and the signature must be confirmed unique to corruption before anything
> is touched.

> **AMENDED 2026-09-19 (Kyle's ruling, after two Step-0 reads). ROW 9 IS SPLIT AND
> REMOVE-ONLY.** The journey roadmap governs over `Vara_Today_IA_Restructure_Roadmap_v2.md`'s
> every-protocol framing, which is treated as **broader product intent rather than current
> implementation scope**. No behavioural/practice discriminator is built for this slice.
>
> **THE SPLIT.** **9.1a**, the write path and the data model, no new surface. **9.1b**, the
> Remove behavioural screen, **WHICH R3 UNBLOCKS ON**. **9.2**, the former Tier 2
> (scheduling, the third card state, the one-off reminder). **9.3**, the former Tier 3a
> (the replacement-flow reminder).
>
> **THE FORMER TIER 3b - auto-mark-done through the `supportingPracticeIds` bridge - IS OUT
> OF THE ACTIVE SEQUENCE** and returns to the backlog with its reasoning preserved: it fires
> for two protocols out of twenty-one, both Recover, both outside this row's Remove fence.
>
> **WHY THE SPLIT.** The data-model work is where an unrecoverable mistake lives and the
> screen is where a visible one does; building the screen on an unproven write path spends
> that distinction.
>
> **R3 UNBLOCKS ON 9.1b, NOT ON ALL OF ROW 9.** R3 will restyle the EXISTING two-state Today
> card; 9.2 owns extending it with the third scheduled state, and that is an **ACCEPTED
> FUTURE UI DELTA, not a prerequisite**.
>
> **9.1a ships "Mark it done" and no "Do it now"** - completion stays user-declared or
> naturally completed per §9 R7, and a control must not claim an outcome it did not produce.
>
> **THE GATES CELL CITED A DOCUMENT THAT DOES NOT EXIST.** Row 9's Gates cell read
> "Completion semantics decision (mockup v1 E1 open item)". A repo-wide search of every doc,
> both mockup HTML files and the archive found **no item labelled "mockup v1 E1" anywhere**.
> The decision it referred to is **§9 item 7**, resolved 2026-09-10 as user-declared or
> naturally completed, and §9's gate sweep at that date already asserted the link. The
> citation is replaced by the real reference; the dangling label is recorded here rather
> than silently dropped.

> **AMENDED 2026-09-21 (Kyle's ruling of 2026-09-19, landing with the next build slice as
> recorded). `PANRESPONDER-VERIFICATION` IS RECLASSIFIED FROM BEFORE BETA TO
> RECORDED-WITH-REASONING.** The row's marker and scope are left unedited above.
>
> Swipe-to-dismiss is inert in the three sheets built on the shared pattern. **Kyle ruled
> that the gesture is not load-bearing:** every one of the three implements an explicit
> close control, overlay tap and hardware back, all wired to the same dismiss, so **two
> visible routes out exist on each**.
>
> **THE ROW IS NOT DELETED AND THE DIAGNOSTIC EVIDENCE STAYS.** Slice 8's instrumented
> build, the two named hypotheses, the killed flattening hypothesis and the working
> reference implementation all remain on the row, so an eventual fix does not re-derive
> them.
>
> **TWO THINGS SURVIVE THE DOWNGRADE.** The handle bar is still rendered on all three
> sheets and still signals a gesture the sheet will not accept - **a lying affordance, a
> separate defect from the dead gesture**, and it either comes off or is recorded as
> known-cosmetic. And **Jen specified all three dismissal routes**; this is Kyle's call to
> make, being an interaction affordance rather than efficacy copy, but **she is to be told
...
### 2026-09-21 - ROLLOVER-SAFETY: a completion can no longer carry yesterday's protocol into today's row

**BUILT `d22d913`, WALKED 2026-09-21 on an iPhone 14 Plus, 9 of 9, zero FAILs. ATTESTED BY KYLE 2026-09-21: suites green at tsc 141 · jest 4048 of 242 · lint 994 errors, 1362 warnings · sentinel 149.**

**THE DEFECT, CONFIRMED BY A RED TEST BEFORE ANY FIX EXISTED.** `useTodayCard`'s load effect sets `setLoading(true)` and awaits, but does not synchronously clear `protocol`, `picked` or `completed`. The hero's mount gate reads `picked && protocol` with no loading gate. So at the render where `syncToday` moves `todayIso`, `markDone` is rebuilt with the new date while `protocol` still holds yesterday's variant — ONE LINE OF THE DEPENDENCY ARRAY APART — and the card is painted, mounted and tappable throughout. The window runs from that render through three to four SEQUENTIAL Firestore round trips, not the one round trip an earlier read estimated.

**THE RED TEST'S FAILURE OUTPUT IS THE DEFECT IN ONE LINE.** Run against unmodified code: `upsertDailyLog("u1", "2026-08-11", { protocolCellId: "refocus-slammed", protocolCompleted: true, completionSource: "user_declared", practiceIds: [] })`. Tuesday's row carrying Monday's variant. TWO FIXTURE CONDITIONS WERE NECESSARY AND NEITHER IS OBVIOUS: Monday had to be picked and NOT completed, because `markDone`'s existing `completed` guard refuses before the defect can occur; and Monday had to be `slammed` so the two days resolve different variants, because otherwise the stale write carries the right id by coincidence and the test passes for the wrong reason.

**NOTHING DOWNSTREAM CAUGHT IT.** On a new unpicked day `prior` is undefined, so all three `stampProvenance` clauses pass and the service stamps a well-formed, indistinguishable-from-real provenance record naming the wrong protocol. Clause (c) protects rows that are ALREADY complete; this row is fresh. The fabrication arrives from a direction that guard does not cover.

**AND THE CORRUPTION IS DURABLE, NOT TRANSIENT.** `markDone` writes no `dailyTimeBudget`, and `hasPickedToday` reads `!!log?.dailyTimeBudget`, so the bad row reads back `picked: false, completed: true`. The user is shown the pick prompt, answers it, `confirmPick` bumps `reloadToken`, the load re-runs, and the hero mounts ALREADY COMPLETE for a day they never acted on, stamped with yesterday's variant.

**THE FIX IS OPTION C: A DATE STAMP COMMITTED IN THE SAME REACT BATCH AS THE PROTOCOL.** `loadedForIso` is set beside `setProtocol(resolved)` with the `todayIso` the async closure captured, so the two can never disagree. `staleDate = loadedForIso !== todayIso` is DERIVED DURING RENDER, which is the whole mechanism: `setLoading(true)` happens in a passive effect and cannot reach the frame React paints between the render where `todayIso` moves and the effect flushing. Deriving it during that render means both guards exist in the first painted frame. THE LOADING-GATE AND SYNCHRONOUS-CLEAR OPTIONS WERE CONSIDERED AND REJECTED FOR EXACTLY THIS REASON: both NARROW the window and neither CLOSES it.

**TWO GUARDS, ONE FLAG.** The render-time check makes the CTA non-actionable via the existing `saving` treatment. The completion-boundary guard in `markDone` is retained regardless of UI state, so a tap already in flight when the day turns still refuses.

**DATE ONLY, NEVER IDENTITY, AND THE CORRECT SAME-DAY BEHAVIOUR ARRIVES BY THE GUARD NOT FIRING.** A same-day `revisionToken` bump leaves `todayIso` untouched, so `staleDate` stays false and the user completes the protocol they read. There is deliberately no comparison of protocol identity anywhere. 9.1a's recorded intent governs: record what the card rendered, never what is true at the tap. A LATER READER MUST NOT "COMPLETE" THE MECHANISM BY ADDING AN IDENTITY COMPARISON — the absence is the design.

**ONE EDIT WENT BEYOND THE CONTRACT'S LETTER AND WAS APPROVED BEFORE THE BUILD.** The contract's render-time clause spoke only of the CTA, but its guarantee was that no false completion state is shown. Those diverge: a stale load carries yesterday's `completed` too, so the card would otherwise render a check and an acknowledgment for a day nobody has completed. Suppressing BOTH branches costs a user who completed yesterday a dimmed "Mark it done" for a few seconds after midnight instead of their done-row. Kyle accepted that: a state that is merely not-yet-known is better than one that is wrong.

**A MUTATION SURVIVED THE FIRST BATTERY, AND CLOSING IT ADDED THE ONE TEST NOBODY HAD SPECIFIED.** Replacing `setLoadedForIso(todayIso)` with `setLoadedForIso(toIsoDate(new Date()))` — stamping the live clock instead of the captured date — passed all thirteen tests. The two are identical whenever a load commits on the day it started, which was every authored case. The only case that distinguishes them is a load resolving AFTER the date moved, which is the superseded-load path. This row claims the captured-date stamp as its mitigation of that path's WRITE consequence, and nothing tested the claim. A case was added that pins the stamp's semantics and kills the mutation. IT SAYS EXPLICITLY THAT IT IS NOT A FIX FOR THE SUPERSEDED-LOAD DEFECT: the stale load still commits, only the write is refused.

**SIX MUTATIONS, ALL KILLED, INCLUDING THE OVER-TIGHT-GUARD CHECK.** Pinning `staleDate` true kills three normal-path cases, the preserved post-settle test among them.

**THE WALK CONFIRMED THE OPPOSITE RISK IS ABSENT AND CHARGED LESS THAN THE CONTRACT ALLOWED.** A1 showed a full-strength CTA on a normal day; B2 showed the tap works after a transition, so the guard releases rather than latching. B1, the observation Kyle asked for, produced NO VISIBLE CHANGE AT ALL across a `revisionToken` bump — the several-second dim he accepted in advance was not charged on this device. Recorded as an observation, not a guarantee.

**TWO LIMITATIONS ACCEPTED AND RECORDED SO NEITHER READS AS AN OVERSIGHT.** "The existing loading presentation" has no referent in this tree — nothing on this surface is bound to `useTodayCard`'s `loading` field and `LoadingSpinner` is not used here — so the fix reuses the `saving` treatment, which is the only in-card treatment for a control that is not actionable right now. And yesterday's `dailyAction` text stays visible during the window, because replacing it would need a card-level skeleton this row explicitly bars.

**PATH 7 IS UNTOUCHED AND HAS ITS OWN APPROVED ROW.** `activeRef` is a single shared boolean, and React runs the previous cleanup before the next effect body, so a superseded load's cleanup sets it false and its successor immediately sets it true again — after which the superseded load commits its stale values, with no closing edge. Option C mitigates the WRITE consequence and does not repair the stale-state commit. THAT ROW ALSO BLOCKS 9.1b.

**BASELINES.** tsc 141, and the sorted list is byte-identical once line and column numbers are stripped — the single apparent difference is `DashboardScreen.tsx(837,33) → (845,33)`, the same error displaced by an eight-line comment insertion. jest 4038 of 242 → 4048 of 242. lint 994 errors / 1362 warnings, unchanged. sentinel 149 — no new strings. `dailyCapacity.test.ts` is untouched and `dayRollover.test.ts:172` is unedited.

### 2026-09-21 - slice 9.1a: completion carries provenance, and the historical rows are proven untouched

**BUILT `cb2eb0f`, walk script `e395c34`, WALKED 2026-09-21 on an iPhone 14 Plus, 11 of
17, zero FAILs, 6 NOT RUN. ATTESTED BY KYLE 2026-09-21: suites green at tsc 141 · jest
4038 of 242 · lint 994 errors, 1362 warnings · sentinel 149.**

**WHAT SHIPPED.** Four additive optional fields on `DailyLog` - `completedAt`,
`completionSource`, `protocolCellId`, `protocolFamily` - written by the existing
`markDone` path from the existing Today card. No new screen, no new route, no new
control, no string that was not on screen the day before. `DailyLog.protocolCompleted`
became optional in TypeScript to match what is actually persisted. A shared test fixture,
the historical-compatibility tests, and three documentation corrections landed with it.

**THIS SLICE CHANGES PERSISTENCE AND CHANGES NO PIXELS, AND THAT IS WHY THE CONSOLE
INSPECTION WAS ACCEPTANCE RATHER THAN A NICETY.** Sections A and B of the walk would have
passed on main. Everything 9.1a adds is invisible on device. A walk that only looked at
the phone could not have passed or failed it, and the script said so on its first page.

**PROVENANCE IS GATED AS A UNIT, NOT FIELD BY FIELD, AND THAT WAS A STEP-0 CORRECTION TO
THE APPROVED CONTRACT.** The contract as written gated only `completedAt` on three
clauses and left the other three fields unconditional. CC's Step 0 found that this
reproduces the exact failure clause (c) exists to prevent, one field over: a second
`markDone` on a pre-9.1a completed row would have written today's identity and source
onto a completion that happened weeks ago while correctly withholding the timestamp,
producing an internally incoherent row. Kyle approved gating all four together under one
condition named `stampProvenance`. THE ARGUMENT THAT JUSTIFIES CLAUSE (c) DOES NOT STOP
AT THE TIMESTAMP.

**THE THREE CLAUSES, AND WHICH ONE DOES THE WORK.** Provenance is written only when (a)
the patch asserts `protocolCompleted === true`, (b) the stored document has no
`completedAt`, and (c) the stored document is not already complete. Mutation-checking
showed (b) and (c) catch different writes: removing (c) reds the historical-row
protection test plus one other; removing (b) reds exactly one test and NOT the protection
test. (c) is load-bearing today; (b) covers a console or Admin-SDK write and a future
second writer. A later reader must not delete (c) after observing that (b) alone passes.

**THE SERVICE OWNS THE TIMESTAMP AND DROPS THE CALLER'S KEYS.** `completedAt` is never on
`DailyLogInput` and is in `stripOwnedKeys`, so a caller cannot forge one. The other three
are accepted from the caller but dropped by the service when `stampProvenance` is false -
the service decides, not the caller, because a caller deciding whether its own write was
the establishing one would re-derive the rule badly.

**IDENTITY IS CAPTURED AT RENDER, NEVER RE-DERIVED AT WRITE TIME, AND THE WALK PROVED
IT.** `markDone` reads `protocol?.id` and `protocol?.family` from the resolved variant the
card rendered and lists those two primitives in its dependency array rather than the
protocol object, which is fresh on every load. Nothing on the write path calls
`selectProtocol`. Step 12 compared the stored `remove-normal` / `behavioral` against "Make
it harder to reach" recorded from the screen at step 1, with `Normal` and `5 min or less`
answered in the picker. They matched.

**AN ABSENT IDENTITY IS A VALID ROW, BY DESIGN.** `protocol` is null on the load-failure
path and `family` is undefined on Recover and Refocus variants, so the identity keys are
written by conditional spread. `ignoreUndefinedProperties` is false, so an explicit
undefined would throw rather than be dropped. A completion with no identity is the
historical-read contract applying to rows written today, not only to legacy ones.

**THE HISTORICAL-ROW PROTECTION IS VERIFIED IN PRODUCTION.**
`qFQy5IMvDoedal69sz7NIukd1Jv1_2026-09-07`, identified on main before the branch build was
installed, was re-opened after the walk and still carries all four provenance fields
ABSENT beside `protocolCompleted: true`. A synthetic record was explicitly not accepted as
proof; the automated tests use one, the walk did not.

**SIX STEPS NOT RUN. NONE TESTS A PROVENANCE FIELD.** Steps 6, 8, 16 and 17 each need a
fresh uncompleted day and the walk gets one per real day; Kyle declined the device-clock
change that is the only other mechanism. Step 4 needs five completed days in a phase that
began 2026-09-07. Step 9's fallback depends on step 4. Step 9's own subject, a 2026-09-20
completion written deliberately on main to serve as a rendering subject, was no longer
today by the time the walk ran, and Today renders only today. The historical read is not
unverified as a result - step 14 inspected the row directly, which is stronger than the
derived-count inference step 9 would have supplied.

**THE LINT BASELINE MOVED AND IT IS ACCEPTED RATHER THAN CLOSED.** 1358 -> 1362 warnings,
zero new errors. All four are in test files: two `max-lines` crossings and two
`no-explicit-any` on the casts that forge a `completedAt` a caller cannot legitimately
supply, matching the cast the existing strips-owned-keys test already uses. Section 17's
"introduce none" is knowingly excepted here, because the casts exist to construct a value
the type system correctly prevents. THE NEXT SLICE INHERITS 1362.

**ONE MID-BUILD TSC REGRESSION, CAUGHT BY THE DIFF.** The new fixture first imported
`CapacityTier` and `TimeClass` from `types/models`, which does not re-export them: TS2459
x2, 141 -> 143. Fixed by routing through the engine barrel, the path
`dailyLog.service.ts` already takes. The sorted-list diff is what surfaced it. This is the
second slice in a row where that rule paid for itself.

**THREE DOCUMENTATION CORRECTIONS, ALL WIDER THAN THE CONTRACT SPECIFIED.** (a) The "a
stored protocolId would be a second copy of an answer that can already be recomputed"
rationale in `models.ts` and `dailyLog.service.test.ts` was amended: the premise is false
for a PAST day, because recomputation needs six inputs and the row stores two, and for
Remove specifically `(capacity, timeClass)` narrows to three candidates and cannot choose
between them. The general rule is unchanged; this is an APPLICATION of it, not an
exception. The `not.toHaveProperty('protocolId')` assertion was kept. (b) The
seeded-capacity claim was corrected at FOUR sites, not the two the contract named - CC's
Step 0 found two more, and leaving two of four asserting the opposite would have been
worse than correcting none. Whether any seeded-capacity row exists in production is
UNKNOWN: it depends on whether a build shipped inside a roughly seven-hour window between
merges `530cfaa` and `6da51cc` on 2026-08-11, which is deploy state and not inferrable
from this repo. It now reads as a tolerated assumption the predicate is built to be
correct under, not as evidence. (c) The slot-not-content limitation is written on the two
identity fields: the pair records WHICH SLOT was served, never which authored words the
user read. Copy changes under a fixed key - 7i replaced twelve protocols' copy and 7k
amended a phrase, neither moving a cell or a family. Accepted, not a gap, and a
content-version field was explicitly not built.

**NO SEPARATE DEV DATABASE EXISTS.** One Firebase project is configured; the test
account's rows are production rows. Recorded because a later walk author will otherwise
assume an isolated environment.

**BASELINES, RE-MEASURED AFTER THE LAST CHANGE.** tsc 141, sorted lists byte-identical.
jest 4008 of 242 -> 4038 of 242, all passing. lint 994 errors / 1362 warnings. sentinel
149, unchanged - 9.1a adds no user-facing string.

### 2026-09-19 - Section B walked partial: the two highest-risk steps pass, three go unrun, and R3's gate (b) clears by ruling

**WALKED BY KYLE, 2026-09-19, iPhone 14 Plus, dev client, default and 1.3x Dynamic Type.
PARTIAL. Attested 2026-09-19.**

**THE RESULT THE ROW EXISTED FOR.** Step 4's `LoginScreen` and step 11's paywall price rows were
the two steps the roadmap flagged highest-risk, and neither had ever been seen rendered under
the text primitive. Both pass at 1.3x. Paper text renders Inter across button label, input text,
placeholder and helper text. Both price rows render Bold price against Regular suffix, visibly
distinct.

**STEPS 8, 9 AND 14 CLAUSES 1 AND 2 PASS AT 1.3x** on every surface that still exists. No
clipped ascender or descender, no fixed-height container clipped, no button truncated. That
matters more than it sounds: R1a applied `Typography.maxFontScale` app-wide where it had been
set at 17 sites, so a container that used to clip only above 1.3x would now clip at it.

**THREE STEPS NOT RUN, NAMED SO THEY ARE NOT MISTAKEN FOR PASSED.** Step 1 (`DevTypography`
diagnostic), step 3 (cold start, system-font flash) and step 13 (Reduce Motion). All three are
runnable on the device present. They are NOT RUN, not not-walkable.

**THREE SUBJECTS WENT MOOT MID-WALK, AND THIS IS A PRODUCT DECISION RATHER THAN A WALK
OUTCOME.** Habits are no longer an accessible part of the app and will not be present at launch
(Kyle, 2026-09-19). `HabitDetailScreen` leaves step 4 and step 8's five-screen list;
`WeeklyHabitGrid` leaves step 9's six tightest ratios, which was therefore walked against five.
Today still renders a "This week" habits card with an "Add a habit" link; Kyle's direction is
that it comes out in a later version and it is deliberately NOT rowed here.

**ONE APPARENT CLIP INVESTIGATED AND DISMISSED.** A scrolled paywall capture showed the
heading's top sliced. Confirmed on device as the header passing under the status bar mid-scroll,
intact at rest. Recorded rather than dropped, because step 8's subject is clipped ascenders and
a later reader of the captures would otherwise re-raise it.

**R3's GATE (b) CLEARS BY RULING, NOT BY A FULL WALK.** Kyle ruled on 2026-09-19 that the
partial suffices. The gate protected against Inter's metrics having silently broken a shipped
layout under two merged slices; the walk tested exactly that and it held. A diagnostic screen, a
splash timing check and a motion-preference check do not bear on it. The ruling is recorded as a
ruling so a reader can evaluate it rather than inherit a claim that fourteen steps were walked.

**GATE (a) IS STILL OPEN AND R3 DOES NOT START.** `todayBackground.webp` was measured on
2026-09-19 and is byte-identical to the file committed at `94c3a77`: 941 x 1672 at aspect 0.563,
against §8.1's 1290 x 2796 at 0.461. Three of five spec items fail. A replacement was believed to
be in the tree and was not.

**THE BOARD NOW CARRIES NO LIVE [Next].** It came off `R1A-SECTION-B-WALK` at this commit and
was not promoted to another row. Row 9 still carries no marker and is still not given an
...
### 2026-09-18 - slice 8 merged: the row is on Today, and it is on the board with its failures attached (merged `8621ec9`; this entry the docs)

**MERGED `8621ec9` ON 2026-09-18**, `--no-ff`, two `-m` flags, **two parents
`b240133` and `b78a97d`**, **six commits from `2185d83` to `b78a97d`**. Branch
`journey/slice-8-good-moments` is **not deleted**. Not pushed at this commit; the
push is Kyle's.

**ALL SIX SUITES RE-VERIFIED ON MERGED `main` AFTER THE MERGE**, not carried over
from the branch: tsc **141** · jest **4008 of 242** · lint **994 errors / 1358
warnings** · sentinel **149** · rules **201 passing, 2 skipped** · functions
**55**. The last two were run under the emulator because the merge brought
`firestore.rules`, the `deleteAccount` manifest and both deletion suites onto
`main` for the first time.

**THE MARKER CARRIES THE FAILURES, AND THAT IS THE POINT OF THIS COMMIT.** Row 8
now reads `[DONE 8621ec9, 2026-09-18; suites attested 2026-09-18; walked 19 of
23, 14 Plus, 3 FAILs (swipe-to-dismiss, see PANRESPONDER-VERIFICATION), 1 NOT RUN
(offline failure, not reachable on a dev client)]`. **A reader of §5 sees that
this row shipped with a known broken gesture without opening the build log.**
Burying a shipped deviation one document down is how it stops being a deviation
and starts being the way things are.

**WHAT SHIPPED WITH A KNOWN DEFECT, STATED ONCE MORE AT THE MERGE SO IT IS NOT
LOST IN THE SEQUENCE.** Swipe-to-dismiss is inert. **UI Standards 10.5 mandates
dismissal by swipe, overlay tap and an explicit control; Jen specified all three
routes; two of three work.** The handle bar remains on screen and is currently an
affordance that does nothing. **This is a shipped deviation from the standard and
from the content owner's spec. It is not deferred work**, and the gesture code is
left in place - recorded as non-functional rather than deleted - so
`PANRESPONDER-VERIFICATION` fixes a wiring problem rather than rebuilding a
feature.

**THE ORDER THIS COMMIT DEMONSTRATES, BECAUSE IT KEEPS COMING UP.** A marker
cannot carry a hash for a commit that does not exist yet. The branch's last docs
commit (`b78a97d`) carried everything the marker was asked to carry EXCEPT the
merge hash, the merge produced the hash, and this commit on `main` swaps `BUILT`
and `UNMERGED` for `DONE 8621ec9`. **That is slice 7c's order** - `247cdc4` on
the branch, then `4fa3882`, then `b240133` on `main` - **and it is the only order
that can work.** A merge message asking for the marker swap afterwards is the
mechanism that stops it being forgotten at the push; this one said so in as many
words.

**ONE THING IS LEFT UNRESOLVED ON PURPOSE AND IS FLAGGED RATHER THAN DECIDED.**
The `[Next]` marker still sits on row 8. Its own condition was *it stays here
until this row merges*, and that condition is now met, so it is owed a move.
**It has not been moved because where it goes is a board decision, not a
bookkeeping one:** table order puts row 9 next, row 9 carries no marker at all,
and there is a standing instruction not to give it an invented one. **Recorded
rather than left silent, so the board does not quietly end up with two live
markers or none.**

**WHAT THIS SLICE LEAVES BEHIND, BEYOND THE FEATURE.** Seven rows on the board -
`TOAST-RELIABILITY`, `GRATITUDE-COPY-SWEEP`, `LEGACY-MOMENT-COLLECTIONS`,
`STANDARDS-WCAG-RECONCILIATION`, `TODAY-NOTICING`, `CONTENT-PACK-7N-STRINGS` and
`PANRESPONDER-VERIFICATION`. Four rules in `mobile/CLAUDE.md`, every one of them
paid for on this branch: re-measure all four baselines after the LAST change; a
green jest run is not a typecheck; a mutation-checked test proves sensitivity and
not reachability; and `findstr`, not `grep`. A retracted tsc baseline that is
corrected in the log rather than amended out of the commit that carried it. And a
walk record corrected from 20 of 23 to 19 of 23 **because the first version
understated its own failures**, which is the same error class as the baseline and
was fixed for the same reason.

---

### 2026-09-18 - slice 8 walked: two of three dismissal routes, a baseline that was never true, and a gesture that was dead in three components before this slice existed (`915b3b9` the test fixes, this entry the docs; **WALKED 2026-09-18 on an iPhone 14 Plus, 19 OF 23 WALKED, 3 FAILs, 1 NOT RUN; ATTESTED 2026-09-18, UNMERGED**, script and results at `docs/walks/8/WALK.md`)
...
**THE WALK: 19 OF 23, THREE FAILs, ONE NOT RUN. NOT ROUNDED.**

**CORRECTED 2026-09-18, AND THE CORRECTION IS ITSELF THE RECORD.** This entry
first read **20 of 23, one FAIL, two NOT RUN**. That **understated the failures
twice over**. **THREE steps test the swipe, not one** - 19b, 20 and 23 - and all
three fail on this build. Step 20 was written down as "could not be reached",
which disguised a third failure as an absence; it exists precisely BECAUSE the
gesture is dead, so its failure is the observation, not a gap. And step 1 turned
out to be a PASS, so it comes off the NOT RUN list. **A record that understates
its own failures is the same class of error as the tsc baseline this slice
already had to retract, and it is corrected here for the same reason.**
...
**STEPS 20 AND 23 ARE FAILS AND WERE FIRST RECORDED AS SOMETHING ELSE.** Step 20
asks whether the sheet follows the finger and snaps back; it does neither, and
that is a failure observed directly rather than a step nobody could run. Step 23
**fails on its swipe clause while its other two clauses pass** - the sheet does
fade rather than slide under Reduce Motion, and "Saved." still appears and still
closes the sheet. Reduce Motion is permitted to remove an animation and not a
gesture; here it removes nothing, because the gesture was already dead with
Reduce Motion off. **So the swipe is tested three times and fails three times.**

**STEP 1 IS A PASS, OUTCOME A, AND IT CLOSES A WEAKNESS THIS ENTRY NAMED BEFORE
IT WAS CLOSED.** Launched online, airplane mode on, pulled to refresh on Today:
**the journey block disappeared, ordinary content remained, and the Good moments
row was still present on scroll.** The route was added to the script after the
walk and then run.

**THE PLACEMENT DECISION IS NOW DEVICE-VERIFIED AND NO LONGER RESTS ON A MOCK.**
`915b3b9` added a screen test for that state using a mocked rejection, and this
entry recorded at the time that it was **a mock asserting a behaviour no device
had confirmed - structurally the same shape as the swipe test, which passed its
mutation check and stayed green while the gesture was dead.** That gap is now
closed by observation. **It is the one place in this slice where the device
agreed with the test**, and it is worth saying so plainly given how much of this
entry is about the places it did not.

**STEP 18 IS THE ONLY NOT RUN, AND IT IS A STRUCTURAL LIMITATION OF THE SETUP
RATHER THAN AN OVERSIGHT** - the same class as the SE half of the device matrix.
A dev client loads its JS bundle from Metro and cannot launch without network,
which is what makes the launch-time offline state awkward to reach.

**THE OBSERVATION THAT MIGHT HAVE STOOD IN FOR IT IS NOT RECORDED, AND THE
WALKER'S OWN ANSWER IS WHY.** The inline failure path was seen incidentally
during the build window, before the rules were deployed, when a genuine server
rejection produced it. Asked what that looked like, **Kyle's answer was "I did
not note it" (2026-09-18).** So **nothing is recorded about it**, and
specifically **it is not written up as "behaved as expected"**. An unrecorded
observation is not evidence. **The inline failure path on this surface remains
unwalked**, and the row ships that way.
...
**ATTESTED BY KYLE, 2026-09-18: suites green on all four.** tsc **141** · jest
**4008 of 242** · lint **994 errors / 1358 warnings** · sentinel **149**. Plus
rules **201 passing, 2 skipped** and functions **55**.

**THE DATE IS OBSERVED, NOT INHERITED**, which is this branch's own rule -
written after an attestation date was taken from the wrong source twice, once
from a row's prose and once from the day a report was read rather than the day
