...
### 2026-09-13 - R1b-ii built: the guards and the lint scope, no pixels move (branch `design/slice-r1b-ii-guards`, FOUR commits, unmerged)
...
requires, and `cardStyles.ts` deleted. **Four commits:** `0b3cda3` the guard, `a5d7db1` the lint
config, `37629db` the deletion, and this entry.
...
**ONE DOC STALENESS LEFT STANDING, NAMED RATHER THAN FIXED.** §7 still reads "R1 adds the lint
that holds the allowlist; **until then it is a review item**." The lint now exists, so that
clause is false. **The build fence for this row was §17 only**, so it was not edited. It is a
one-clause correction for whoever opens §7 next, and it is booked here rather than done quietly
outside the fence.
