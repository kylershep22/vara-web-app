...
| Floating tab bar translucent fill | `src/constants/colors.ts` | `Colors.tabBarTranslucent` (`rgba(250,250,246,0.55)`) - **a declaration, not an alias.** Mist White at 0.55; `colors.ts` carries Mist White at full alpha and at zero and at no value between, so there is nothing to point at. Aliased into `ColorTokens.tabBarTranslucent` per this section's contract. Added in R2 |
...
| `inputHeight` | 48 |
| `headerHeight` | 56 |
| `tabBarHeight` | **56, and NOT what the navigator renders.** Legacy: written for an opaque bar that sat in the layout, never read by the navigator, and kept only because it still has consumers. Retiring it means finding them, which is booked to `docs/TECH_DEBT_BACKLOG.md`. Do not use it for new work |
