...
**Four code keys carry the Muted Sage Gray value. R1d made ONE of them an alias; three are still independent literals (recorded at R1b-i, amended at R1d 2026-09-13).** The four are `Colors.mutedSageGray` (the table row above), `Colors.textSecondary`, `Colors.text.secondary` and `ColorTokens.textSecondary`. R1b-i moved all four by hand to `#56655D` to keep them at one value. R1d's dealias made `ColorTokens.textSecondary` a real alias of `Colors.mutedSageGray`, pinned by `__tests__/designTokenAliases.test.ts`, so **that key can no longer drift.**

**THE R1d ROW PREDICTED THIS NOTE WOULD REDUCE TO "ONE DECLARATION AND THREE REFERENCES". IT DID NOT, AND THE DIFFERENCE IS WORTH STATING RATHER THAN ROUNDING OFF.** It is one declaration, one reference and **two remaining literals**: `Colors.textSecondary` and `Colors.text.secondary` are declared inside `colors.ts` itself, and R1d's scope was the `designTokens.ts` fork, not the palette's internal one. **So a change to Muted Sage Gray is still a change to three places, not one** - down from four. Closing it means making those two aliases of `mutedSageGray` within `colors.ts`, which is a one-line-each edit and is not booked to a row yet.
...
- **R1d: the token mirrors become aliases, and two type errors turn out to have been shipping
  ...
  - **4.1's four-keys note is amended and is NOT closed.** The R1d row predicted it would
    reduce to "one declaration and three references". One key became an alias; two remain
    literals inside `colors.ts`, which was outside this row's fence. Three places, down from
    four. The note says so rather than claiming the win.
