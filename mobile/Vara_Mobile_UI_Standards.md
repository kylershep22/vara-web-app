...
`Insights`, `FocusTimer`, `Journal`, `Breathwork`, `BreathworkDetail` **(DARK)**, `Sleep`, `SleepDetail` **(DARK)**, `Movement` **(DARK)**, `MovementDetail` **(DARK)**, `Masterclass`, `MasterclassDetail`, `PodcastEpisode`, `HelpSupport`, `WearableIntegration`, `HabitDetail`, `NotificationOptIn`, `CheckInFlow`, `Practices`, `PracticeRun`, `FocusDayBlocks`, `FocusTasks`, `EnergyBrowse`, `FocusRhythms`, `WeeklyEntry`, `WeeklyFloor`, `WeeklyClose`, `EmailVerification`.

**DARK means REGISTERED BUT UNREACHABLE, and it is marked here because a surface type on a route no one can open is a promise the walk cannot check (recorded at R1d, 2026-09-13).** All four are `Stack.Screen`s on `AppStack` with real `options`, so they read as live to anyone grepping the navigator. Nothing navigates to them: `SleepDetail` and `Movement` have zero `navigate()` callers anywhere in `src/`, and `MovementDetail` and `BreathworkDetail` are reached only from `Movement` and `Breathwork`, which are themselves unreached. **The one apparent escape hatch is also dead:** `utils/getNudgeSuggestion.ts` names `ROUTES.Breathwork`, its result is computed in `useDashboard`, and no component renders a `NudgeSuggestion` - so the nudge cannot deliver anyone to it either. `Breathwork` and `Sleep` are dark on the same evidence and keep their rows unannotated only because R1d did not touch them; the row that next edits either one owes this note.

This is why R1d fixed a bottom-padding defect on five screens and walked one. **A dark route that is ever relit is walked then, and the fix is verified then** - see the R1d row's walk cell and `docs/walks/r1d/WALK.md`, which records steps 4 and 5 as deferred rather than passed.
...
**`designTokens.ts` MIRRORS NOTHING. IT ALIASES (rewritten at R1d, 2026-09-13).** Every value it exports is the canonical value, by reference to the canonical key, with the three declarations in the rows above as the only exceptions - each of which exists because the canonical scale has no key for it, not because the two have drifted.

**THE SENTENCE THIS REPLACES WAS THE PROBLEM, NOT A DESCRIPTION OF IT.** It read "two mirrors exist… where they disagree, `Layout` and `Typography` are canonical and the flat mirror is corrected to match; known splits are listed in 16" - a rule for reconciling a fork, which licensed the fork continuing to exist and pointed at a list in 16 that was never written. `ColorTokens` and `TypographyTokens` were independent literal copies, and four had already drifted by SPELLING rather than by hue: `ColorTokens.surfaceTintedLight` was `'rgba(213, 227, 209, 0.5)'` against `Colors.dewSageLight`'s `'rgba(213,227,209,0.5)'`, plus `primaryLight`, `primaryMedium` and `disabled` the same way. Identical on screen, unequal to `===`, invisible to any walk. There is now nothing to reconcile and no split list to keep: `__tests__/designTokenAliases.test.ts` asserts every aliased key against its canonical AND asserts the alias map covers every key in both objects, so a new literal fails rather than arrives.
...
**Muted Sage Gray is ONE DECLARATION AND THREE REFERENCES (recorded at R1b-i as four literals; closed at R1d, 2026-09-14).** The declaration is `MUTED_SAGE_GRAY` in `src/constants/colors.ts`. The three references are `Colors.mutedSageGray` (the table row above), `Colors.textSecondary` and `Colors.text.secondary`; `ColorTokens.textSecondary` in `designTokens.ts` is an alias of `Colors.mutedSageGray` and is the fourth. **A change to this colour is now a one-line edit.**

**WHAT IT TOOK, BECAUSE THE HISTORY IS THE ARGUMENT FOR THE RULE.** R1b-i found the value failing AA at `#6F7F77` and had to move it **four separate times by hand** - the four keys were independent literals, and moving only the key this table names would have left the majority of the surface below AA and put two near-identical greys on screen together in 17 files. R1b-i recorded the warning that until they became real references, "a change to one of the four is a change to one quarter of the colour". R1d's dealias closed the `designTokens.ts` fork; the widening that followed closed the two inside `colors.ts` by lifting the value to a module const the three keys read. **An object literal cannot reference itself**, which is why the declaration sits one level above `Colors` rather than being written as `Colors.mutedSageGray` in each key.

**`__tests__/designTokenAliases.test.ts` HOLDS IT, AND HOLDS THE MECHANISM AND NOT ONLY THE VALUE.** It asserts all four keys against the value, asserts them against **one another** (so the test's own literal drifting with the palette does not pass), and asserts that `colors.ts` contains the hex **exactly once** - because re-inlining a literal into one of the three keys re-forks the colour while every value assertion stays green. That is the silent-failure shape R1b-i hit from the other direction, and the value tests alone do not catch it.

**`Colors.textSecondary` is the larger consumer** (412 sites across 116 files, against `mutedSageGray`'s 336 across 125), so a reader who greps only the table's key still finds under half the surface. The tab bar's `tabBarInactiveTintColor` reads `Colors.textSecondary`, which is why "inactive icons" in the role column is reached through that key and not through `mutedSageGray`.
...
| Timer | `timer` | **48 or 52, see below** | 500 | `TypographyTokens.letterSpacingTimer` (-0.02 em, multiplied by the font size at the call site), tabular | Teal | Session timers and durations only |

**TWO TIMER SIZES SHIP, AND THIS ROW USED TO RECORD ONE (found at R1d's Step 0, 2026-09-13).** `ActiveRoutinePlayer` renders **48** via `TypographyTokens.fontTimerPlayer`, which is an alias of the canonical `Typography.fontSize.timer`. `PomodoroTab` renders **52** via `TypographyTokens.fontTimerLarge`, which has no canonical counterpart and is a declaration for that reason. This table said 48, so a reader sizing a third timer against the document would have matched one of the two shipping timers by accident. **It is recorded, not resolved:** collapsing them is a rendered change on a live screen, which a substitution slice does not get to make, so **the decision is deferred to the Focus surface slice** - which owns `PomodoroTab` and is the right place to decide whether the Pomodoro timer is deliberately the larger of the two or is 4pt of drift. Until then 3.3 carries `fontTimerLarge` as a declaration with the same note.

Tracking values are React Native `letterSpacing`, which is absolute points. The keys are `Typography.letterSpacing.tighter` (-0.5), `tight` (-0.25), `normal` (0), `wide` (0.5), assigned directly. One exception exists: `TypographyTokens.letterSpacingTimer` is an em value (-0.02) and must be multiplied by the font size at the call site, as `PomodoroTab` and `ActiveRoutinePlayer` do. Assigning it directly yields -0.02pt, which looks like no tracking rather than a bug. Never add a second em-denominated token; new tracking goes on the points scale. **R1d's dealias left this key a declaration for exactly that reason: an em ratio is not comparable to a points scale, so there is no canonical key it could have aliased to, and aliasing it to one would have been the second em-denominated token this sentence bars.**
...
- Maximum line length 65 to 75 characters, read as a **CEILING** and not as a target band (clarified 2026-09-19). No style in the scale approaches the upper limit on any shipped device: measured against the shipped Inter faces over a corpus of live Vara copy, `body` at 1.3x Dynamic Type yields about **31** characters per line at 375pt and about **41** at 430pt. Reaching 65 characters in the 343pt available at SE width would require a rendered size near **11.35pt**, below every entry in 5.2. The rule's work is therefore to stop anyone widening a text column or shrinking type to fit more in. Do not reduce margins to fit more. *(The earlier wording claimed the band was "naturally satisfied" on a 390pt screen at 16pt with 16pt margins; that configuration yields roughly 48 characters, so the band was unreachable and the claim was false. Corrected in place rather than appended because the sentence stated a measurement, not a decision.)*
...
- **Touch targets**: **48 by 48 is the floor** for every interactive element (Apple's 44 is the minimum; Vara adds margin). 8 minimum between adjacent targets. Small text links get hit slop to 48. **Two constants carry it, both in `src/constants/designTokens.ts`:** `SizeTokens.touchTargetMin` and `SizeTokens.inputHeight`. **There used to be a third and forty-two unofficial ones (closed at R1d, 2026-09-13).** `MIN_TOUCH_TARGET_SIZE` lived in `src/utils/accessibility.ts` and went with that file; separately, **forty-two files each declared their own module-local `const MIN_TOUCH_TARGET = 48`** across 100 style sites, none exported, none reachable from here. All forty-two now read `SizeTokens.touchTargetMin`, so raising the floor is one edit rather than forty-five. v2.0 said two of the constants "still read 44 and are corrected as part of adopting this document" and named the wrong directory; v2.1 corrected both halves of that.
- **Screen reader**: every pressable carries `accessibilityRole` and a meaningful `accessibilityLabel` that names the action, not the appearance ("Save reflection," not "Green button"). Checkboxes carry `accessibilityState`. Decorative images are `accessible={false}`. Progress indicators are `progressbar` with a value. **These props are written inline on the element. That is the app-wide pattern and the one this document asks for: 289 `accessibilityRole` sites across 142 files, 319 `accessibilityLabel`, 45 `accessibilityState`.** Reading order matches visual order; nothing relies on position alone.
- **On the prop-builders this section used to mandate (retired at R1d, 2026-09-13).** The sentence above read "use the builders in `src/utils/accessibility.ts` (`buttonA11yProps`, `checkboxA11yProps`, `progressA11yProps`, `switchA11yProps`, `headerA11yProps`) so labels are consistent." **Nothing ever called them, in the whole life of the app** - all seventeen exports of that file had zero importers - and the file is now deleted. **It is retired rather than adopted because every rule the builders encoded is already in the bullets above**, in prose, and three of the pieces were wrong on their own terms: `getProgressLabel` returned "3 of 10 items completed, 30%", a denominator and a percentage, both barred by the non-negotiables; `getContrastRatio` parsed six-digit hex only and returned `NaN` for every `rgba()` value in 4.1, which is half the palette; and `headerA11yProps(level)` took a heading level and discarded it. **Do not reintroduce a builder layer without a consumer.**
...
**Two further numbers are corrections and are stated as such.** The raw-hex figure is the eslint run's own output (`no-restricted-syntax`), not an estimate; an earlier working figure of 331 circulated and is wrong. It read 501 of 1100 errors at `c30671c`; **R1b-ii closed the configuration gap and it read 431 of 1033; R1b-i retired 35 of them and it read 396 of 995; R1d left the raw-hex figure at 396 and took the total to 994.**

**R1d's single error is named so it is not read as a re-measurement (2026-09-13).** It is `@typescript-eslint/no-unused-vars` at `src/utils/accessibility.ts:204` - `headerA11yProps(level)` took a heading level and discarded it - and it went with that file when 16's prop-builder layer was retired. **No raw hex moved and no rule configuration changed.** Note for anyone reading the raw-hex figure as a survey of colour literals: `no-restricted-syntax` bars raw **hex** and does not see `rgba()` calls, four of which R1d's old-value sweep found still hardcoded where a token exists (`DESIGN_BACKLOG` item 12).
...
>
> **Why these two.** The SE is the narrowest and the shortest current iPhone, so one device is the binding case for horizontal layout at 375pt (hub card rows, nav labels) and for bottom clearance at 667pt scrolled fully down, at the same time. *(Amended 2026-09-19: 5.4's line length was removed from this list. It resolves identically at 375, 428 and 430pt, so it is not SE-binding and never was. The SE remains the binding case for the other items named here.)* The 16 Pro Max is the tallest and the widest. The matrix also spans **both scale factors, @2x and @3x**, which (g) needs: a raster background resolves differently at each.
>
...
- **6.2** amended: the fixed 48 bottom padding is **retired for the routes where the tab bar is visible**, which take their inset from `useBottomTabBarHeight()` per 12.2. The 48 rule stands for any other fixed bottom control. Added so 6.2 and 12.2 do not disagree on the same page.
...
- **R1d: the token mirrors become aliases, and two type errors turn out to have been shipping
  as visual defects (September 2026).** A substitution slice: five items, no rendered value
  moved by design, and the two that did move were already broken. **No rule in this document
  changed.** R1d implements 3.3's single-source contract and 16's touch-target floor against
  code that had been declaring its own copies of both.
  - **3.3's "two mirrors exist" sentence is retired and replaced.** It gave a rule for
    reconciling a fork ("where they disagree, `Layout` and `Typography` are canonical…") and
    pointed at a split list in 16 that was never written - which licensed the fork rather than
    closing it. `ColorTokens` and `TypographyTokens` are now aliases, `designTokens.ts` mirrors
    nothing, and the guarantee is a test rather than a sentence.
  - **3.3 gains three rows, one per deliberate non-alias**: `ColorTokens.secondaryLight`
    (Silver Sage at 0.25, an alpha the palette does not carry),
    `TypographyTokens.fontTimerLarge` (52, against a canonical 48) and
    `TypographyTokens.letterSpacingTimer` (an em ratio, not a point value). Per 3.3's own
    contract, a token in code with no entry here is undocumented drift - and these three are
    the only values in that file that are not reachable from a canonical scale.
  - **5.2's Timer row said 48 and two timer sizes ship.** `ActiveRoutinePlayer` renders 48,
    `PomodoroTab` renders 52. Recorded, not resolved: collapsing them is a rendered change on a
    live screen, deferred to the Focus surface slice.
  - **4.1's four-keys note is CLOSED: one declaration and three references**, which is what
    R1b-i promised and what the R1d row predicted. It took two passes. The first closed only
    the `designTokens.ts` fork, because the row's fence stopped at that file, and the note was
    written to say so - one declaration, one reference, two remaining literals. **Kyle then
    widened the fence to `colors.ts` for two lines** and the other two closed. The value now
    lives in a module const above `Colors`, since an object literal cannot reference itself.
    The guard asserts the mechanism as well as the value: `colors.ts` must contain the hex
    exactly once, because re-inlining a literal re-forks the colour while every value
    assertion stays green.
  - **16 loses the prop-builder mandate.** `src/utils/accessibility.ts` had seventeen exports
    and zero importers for the life of the app; it is deleted and the bullet now describes the
    inline pattern the app actually uses, with the three defects in the builders recorded so
    the layer is not rebuilt. **16's touch-target sentence drops from three constants to two**,
    and gains the forty-two module-local `MIN_TOUCH_TARGET` consts that were never visible from
    this document at all, now consolidated onto `SizeTokens.touchTargetMin`.
  - **2.8's route table marks four routes DARK**: `BreathworkDetail`, `SleepDetail`, `Movement`
    and `MovementDetail` are registered on `AppStack` with real options and nothing navigates
    to any of them. Added because R1d fixed a bottom-padding defect on five screens and could
