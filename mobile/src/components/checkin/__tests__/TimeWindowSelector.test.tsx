...
describe('TimeWindowSelector', () => {
  ...
  // The per-state chip-eligibility filter was removed with the engine wiring
  // (the engine owns slot eligibility; the time budget is a pure preference).
  // All five chips are always shown.
  it('always shows all five chips regardless of any state context', () => {
    const { queryByTestId } = render(<TimeWindowSelector onSelect={jest.fn()} />);
    for (const value of [2, 5, 10, 20, 45] as const) {
