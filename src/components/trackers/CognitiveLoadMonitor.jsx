...
const CognitiveLoadMonitor = ({ userId, compactMode = false }) => {
  ...
  const getHeatmapColor = (level) => {
    ...
    if (level < 9) return 'bg-green-200';
    return 'bg-green-400';
  };
      ...
      <div className="bg-white rounded-xl border border-gray-200 p-4">
        ...
        {showForm && (
          <div className="space-y-3 border-t pt-4">
            <div>
              <label className="text-xs text-gray-600">Energy: {formData.energyLevel}/10</label>
              <input
                type="range"
                min="1"
                max="10"
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <div className="bg-white rounded-xl border border-gray-200 p-6">
            <div className="flex items-center gap-3 mb-2">
              ...
              <span className="text-sm text-gray-600">Today's Energy</span>
            </div>
            <div className="text-3xl font-bold text-gray-900">{todayAvg.energy}/10</div>
            <div className="text-sm text-gray-600">{todayAvg.count} check-ins</div>
          </div>
          <div className="bg-white rounded-xl border border-gray-200 p-6">
            <div className="flex items-center gap-3 mb-2">
            ...
            <div className="text-3xl font-bold text-gray-900">{todayAvg.focus}/10</div>
      ...
      <div className="bg-white rounded-xl border border-gray-200 p-6">
        <div className="flex items-center justify-between mb-4">
          <h3 className="text-lg font-semibold text-gray-900">Energy Check-In</h3>
          {!showForm && (
            <button
      ...
      {checkins.length >= 5 && (
        <div className="bg-white rounded-xl border border-gray-200 p-6">
          <h3 className="text-lg font-semibold text-gray-900 mb-4">Energy Heatmap (24-Hour View)</h3>
                ...
                <div key={checkin.id} className="p-4 bg-gray-50 rounded-lg border border-gray-200">
                  <div className="flex items-start justify-between mb-2">
                    <div className="flex items-center gap-2">
                      ...
                      <div>
                        <div className="font-medium text-gray-900">{mood?.label || checkin.mood}</div>
                        <div className="text-sm text-gray-600">{activity?.label || checkin.activity}</div>
                      </div>
                    </div>
                    <div className="text-right">
                      <div className="text-sm text-gray-500">{formatDate(checkin.timestamp)}</div>
                      <div className="text-xs text-gray-500">{formatTime(checkin.timestamp)}</div>
                    </div>
                  </div>
                  <div className="flex items-center gap-4 text-sm">
                    <div className="flex items-center gap-1">
                      ...
                      <span className="font-semibold text-gray-900">{checkin.energyLevel}/10</span>
                    </div>
                    <div className="flex items-center gap-1">
                      <TrendingUp className="text-blue-600" size={14} />
                      <span className="text-gray-600">Focus:</span>
                      <span className="font-semibold text-gray-900">{checkin.focusLevel}/10</span>
                    </div>
                  </div>
                  {checkin.notes && (
                    <p className="text-sm text-gray-700 mt-2 italic">{checkin.notes}</p>
                  )}
                </div>
              );
            })}
